...
this yields a total emitter resistance of $r_{etot} = 12.5\Omega + 110\Omega = 122.5\Omega$. Thus changes to temperature only affects the term 
with $V_T$, which is $\approx \pm 2mV/C$

The goal here is to relate the differnetial input voltage $V_d$ to the output voltage $V_o$, by elimintating the internal transistor current $i_b$,
...
\begin{align}
    A_{diff} \rightarrow R_E \approx 109.524\Omega
\end{align}
